...
import kotlinx.android.synthetic.main.layout_discview.view.*
import java.util.*
...
class DiscView constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    private val mScreenWidth: Int by lazy { DisplayUtil.getScreenWidth(context) }
    private val mScreenHeight: Int by lazy { DisplayUtil.getScreenHeight(context) }
    ...
    private fun initDiscBlackground() {
        ...
        ivDiscBlackgound.setImageDrawable(discBlackgroundDrawable)

        val marginTop = (DisplayUtil.SCALE_DISC_MARGIN_TOP * mScreenHeight).toInt()
        val layoutParams = ivDiscBlackgound
            .layoutParams as LayoutParams
        layoutParams.setMargins(0, marginTop, 0, 0)

        ivDiscBlackgound.layoutParams = layoutParams
    }
    ...
    private fun initViewPager() {
        ...
        val layoutParams = mVpContain!!.layoutParams as LayoutParams
        val marginTop = (DisplayUtil.SCALE_DISC_MARGIN_TOP * mScreenHeight).toInt()
        layoutParams.setMargins(0, marginTop, 0, 0)
    ...
    private fun resetOtherDiscAnimation(position: Int) {
        for (i in mDiscLayouts.indices) {
            ...
            val imageView = mDiscLayouts[i].findViewById(R.id.ivDisc) as ImageView
    ...
    private fun initNeedle() {
        ...
        val layoutParams = ivNeedle.layoutParams as LayoutParams
        ...
        ivNeedle.pivotX = pivotX.toFloat()
        ivNeedle.pivotY = pivotY.toFloat()
        ivNeedle.rotation = DisplayUtil.ROTATION_INIT_NEEDLE
        ivNeedle.setImageBitmap(bitmap)
        ivNeedle.layoutParams = layoutParams
    }

    private fun initObjectAnimator() {
        mNeedleAnimator = ObjectAnimator.ofFloat(
            ivNeedle, View.ROTATION, DisplayUtil
                .ROTATION_INIT_NEEDLE, 0F
        ).apply {
            duration = DURATION_NEEDLE_ANIAMTOR
    ...
    private fun getDiscDrawable(musicPicRes: Int): Drawable {
        ...
        val discDrawable = BitmapDrawable(resources, bitmapDisc)
        ...
        val musicPicMargin = ((DisplayUtil.SCALE_DISC_SIZE - DisplayUtil
    ...
    fun setMusicDataList(musicDataList: List<MusicData>) {
        ...
        for (musicData in mMusicDatas) {
            ...
            val disc = discLayout.findViewById(R.id.ivDisc) as ImageView
    ...
    private fun getDiscObjectAnimator(disc: ImageView): ObjectAnimator {
        return ObjectAnimator.ofFloat(disc, View.ROTATION, 0F, 360F).apply {
    ...
    companion object {
        ...
        const val DURATION_NEEDLE_ANIAMTOR = 500L
